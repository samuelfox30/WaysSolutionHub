...
@admin_bp.route('/admin/api/relatorio-ia-viabilidade/<int:empresa_id>', methods=['POST'])
def api_relatorio_ia_viabilidade(empresa_id):
    """Gera relatório de viabilidade usando IA (Gemini)"""
    if not ('user_email' in session and session.get('user_role') == 'admin'):
        return jsonify({"error": "Não autorizado"}), 403
