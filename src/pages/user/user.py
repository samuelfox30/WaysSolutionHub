...
@user_bp.route('/user/bpo/pdf')
def dashboard_bpo_pdf_user():
    """Gera PDF do Dashboard BPO da empresa do usuário"""
    if not ('user_email' in session and session.get('user_role') == 'user'):
        flash("Acesso negado. Faça login como usuário.", "danger")
        return redirect(url_for('index.login'))
    ...
    # Criar objeto empresa compatível
    empresa = {'nome': empresa_nome}
    ...
    logger.info(f"Gerando PDF do Dashboard BPO (Usuário) - Empresa {empresa_id}")
